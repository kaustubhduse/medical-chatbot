...
def plot_metric_comparison(metrics_df):
    """Interactive bar chart with reference ranges"""
    if metrics_df.empty:
        return
...
def generate_radial_health_score(metrics_df):
    ...
    fig = px.line_polar(normalized_df, 
                       r='score', 
                       theta='metric',
                       line_close=True,
...
def create_clinical_summary_pdf(metrics_df):
    ...
    for _, row in metrics_df.iterrows():
        ...
        pdf.cell(col_widths[3], 10, txt=row['status'], border=1)
        pdf.ln()
        ...
        if '-' in str(ref_range):
            try:
                ...
                fig.add_hline(y=low, line_dash="dot",
                             annotation_text=f"{metric} Lower Limit",
                             line_color="red")
                fig.add_hline(y=high, line_dash="dot",
                             annotation_text=f"{metric} Upper Limit",
                             line_color="green")
            except:
                pass
